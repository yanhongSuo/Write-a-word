...
drawGrid()
var color_btn = document.getElementsByClassName("color_btn")
for(var i=0; i<color_btn.length; i++){
	color_btn[i].index = i
	color_btn[i].onclick = function(){
		for(var j=0; j<i; j++){
			color_btn[j].className = 'color_btn'
		}
...
function calcDistance( loc1, loc2){
	return Math.sqrt( (loc1.x - loc2.x)*(loc1.x - loc2.x) + (loc2.y - loc2.y)*(loc2.y - loc2.y) )
}
